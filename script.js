...
function desenharGraficoFiltrado(dados) {
  ...
  // Gráfico de barras (coluna D)
  const dadosBarras = new google.visualization.DataTable();
  dadosBarras.addColumn('date', 'Data');
  dadosBarras.addColumn('number', 'Pressão');

  for (let i = 0; i < dados.getNumberOfRows(); i++) {
    const data = dados.getValue(i, 0);
    const linhaOriginal = dadosOriginais.getFilteredRows([{column: 0, value: data}]);
    if (linhaOriginal.length > 0) {
      const valorPressao = dadosOriginais.getValue(linhaOriginal[0], 3); // coluna D
      dadosBarras.addRow([data, valorPressao]);
    }
  }
